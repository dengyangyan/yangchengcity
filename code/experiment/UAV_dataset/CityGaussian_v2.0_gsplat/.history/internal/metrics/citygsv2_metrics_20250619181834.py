...
class CityGSV2MetricsModule(GS2DMetricsImpl):
    ...
    def get_train_metrics(self, pl_module, gaussian_model, step: int, batch, outputs) -> Tuple[Dict[str, Any], Dict[str, bool]]:
        ...
        # 如果当前图像的名称为DJI_20250108093841_1888_V.JPG,则在txt中新增loss的值,涵盖metrics ssim,dist_loss,normal_loss,d_reg,loss
        #有阴影
        if batch[0].metadata.image_name == "DJI_20250108093841_1888_V.JPG":
            with open("DJI_20250108093841_1888_V.txt", "a") as f:
                f.write(f"{metrics['ssim']},{metrics['dist_loss']},{metrics['normal_loss']},{metrics['d_reg']},{metrics['loss']}\n")  
        if batch[0].metadata.image_name == "DJI_20250108093602_1618_V.JPG":
            with open("DJI_20250108093602_1618_V.txt", "a") as f:
                f.write(f"{metrics['ssim']},{metrics['dist_loss']},{metrics['normal_loss']},{metrics['d_reg']},{metrics['loss']}\n")
        if batch[0].metadata.image_name == "DJI_20250108093601_1615_V.JPG":
            with open("DJI_20250108093601_1615_V.txt", "a") as f:
                f.write(f"{metrics['ssim']},{metrics['dist_loss']},{metrics['normal_loss']},{metrics['d_reg']},{metrics['loss']}\n")
        # 无阴影
        if batch[0].metadata.image_name == "DJI_20250107135447_1963_V.JPG":
            with open("DJI_20250107135447_1963_V.txt", "a") as f:
                f.write(f"{metrics['ssim']},{metrics['dist_loss']},{metrics['normal_loss']},{metrics['d_reg']},{metrics['loss']}\n")
        if batch[0].metadata.image_name == "DJI_20250107135448_1966_V.JPG":
            with open("DJI_20250107135448_1966_V.txt", "a") as f:
                f.write(f"{metrics['ssim']},{metrics['dist_loss']},{metrics['normal_loss']},{metrics['d_reg']},{metrics['loss']}\n")
        if batch[0].metadata.image_name == "DJI_20250107135727_2269_V.JPG":
            with open("DJI_20250107135727_2269_V.txt", "a") as f:
                f.write(f"{metrics['ssim']},{metrics['dist_loss']},{metrics['normal_loss']},{metrics['d_reg']},{metrics['loss']}\n")
